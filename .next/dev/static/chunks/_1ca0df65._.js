...
function PostersPage() {
    ...
    const loadPosters = async ()=>{
        try {
            ...
            // For posters without lowResImageUrl, try to fetch it
            // This is a one-time operation to populate missing low-res URLs
            const postersWithLowRes = await Promise.all(postersOnly.map(async (poster)=>{
                // If lowResImageUrl already exists, use it
                if (poster.lowResImageUrl || !poster.imageUrl) {
                    return poster;
                }
                // Try to get low-res URL from storage
                try {
                    ...
                    if (urlMatch) {
                        ...
                        const { url: lowResUrl } = await getFileURL(lowResPath);
                        if (lowResUrl) {
                            console.log(`✅ Found low-res for ${poster.name}, updating Firestore...`);
                            // Update Firestore with low-res URL (fire and forget - don't wait)
                            if (poster.id) {
                                updatePoster(poster.id, {
                                    lowResImageUrl: lowResUrl
                                }).catch((err)=>{
                                    console.warn(`Failed to update Firestore for ${poster.name}:`, err);
                                });
                            }
                            // Use the fetched URL for this session
                            return {
                                ...poster,
                                lowResImageUrl: lowResUrl
                            };
...
//# sourceMappingURL=_1ca0df65._.js.map